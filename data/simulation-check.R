...
# Use mcmc_areas to show full posterior 
# Set the color scheme 
color_scheme_set("gray")

# Start with beta- second-level regression parameters
b1.sim.plot <- mcmc_areas(sim.est.sum$beta, pars = c("beta1"), prob = .9) +
...
ggplot(as.data.frame(sim.est.sum$sigma_all), aes(x = sim.est.sum$sigma_all)) + geom_density() +
  geom_vline(xintercept = c(true.sigma.all), color = "black", size = 2) 


# Clear fit models from workspace
rm(list = c("sim.out", "sim.out.est"))
